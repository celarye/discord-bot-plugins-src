...
use crate::{
    ...
    exports::discord_bot::plugin::{
        plugin_functions::{DiscordEvents, Guest, RegistrationsResponse, SupportedRegistrations},
        plugin_types::{RegistrationsResponseDiscordEvents, RegistrationsResponseScheduledJob},
    },
};
...
impl Guest for Plugin {
    fn registrations(
        ...
        if supported_registrations.interaction_create_commands && settings.cats_on_demand {
            commands.push(
        ...
        for automated_cat in settings.automated_cats {
            // TODO:
            // 1. Prevent duplicate names/ids
            // 2. Write successful items to CONTEXT
            scheduled_jobs.push(RegistrationsResponseScheduledJob {
                name: format!(
                    "automated_cat_{}_{}",
                    &automated_cat.guild_id, &automated_cat.channel_id
                ),
                cron: automated_cat.cron,
            });
        }
        ...
        Ok(RegistrationsResponse {
            discord_events: RegistrationsResponseDiscordEvents {
                ...
                message_create: supported_registrations.message_create
    ...
    fn discord_event(event: DiscordEvents) -> Result<Vec<DiscordRequests>, String> {
        match event {
    ...
    fn scheduled_job(job: String) -> Result<Vec<DiscordRequests>, String> {
        match job.as_str() {
...
impl Plugin {
    ...
    ) -> Result<Vec<DiscordRequests>, String> {
    ...
    fn cat_message(&self, message: Box<MessageCreate>) -> Result<Vec<DiscordRequests>, String> {
        todo!()
    }

    fn automated_cat(&self) -> Result<Vec<DiscordRequests>, String> {
        todo!();
    }
